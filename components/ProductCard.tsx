...
const ProductCard: React.FC<ProductCardProps> = ({ product, onAddToCartClick }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden transform hover:-translate-y-2 transition-transform duration-300 flex flex-col">
      <img className="w-full h-56 object-cover" src={product.imageUrl} alt={t(product.nameKey)} />
      <div className="p-6 flex flex-col flex-grow">
        <h3 className="text-xl font-bold font-serif text-primary mb-2">{t(product.nameKey)}</h3>
        <p className="text-gray-600 text-sm mb-4 flex-grow">{t(product.descriptionKey)}</p>
